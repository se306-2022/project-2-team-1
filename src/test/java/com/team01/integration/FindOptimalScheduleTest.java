package com.team01.integration;

import com.team01.algorithm.BranchAndBoundTest;
...
public class FindOptimalScheduleTest {
    ...
    Stream<DynamicTest> providedTests() throws IOException {
        ...
        return Arrays.stream(input).map(params -> {
            ...
            try {
                GraphController gc = new GraphController(ic.commandLineParser.getInputFileName());
                graph = gc.getGraph();
                Schedule schedule = taskRunner.safeRun(new BranchAndBound(), graph);
                ExportToDotFile export = new ExportToDotFile(graph, ic.commandLineParser.getOutputFileName(), schedule);
